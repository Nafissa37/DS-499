...
#Number of features
n_features <- length(setdiff(names(train_data), c("id", "address_number", "street", "stormwater_benefits", "stormwater_elimination")))


rf_stormwater <- ranger(
  formula = stormwater_elimination ~ . -id -address_number -street -stormwater_elimination -stormwater_benefits -longitude -latitude,
  data = train_data,
...
```{r eval=FALSE}
#Number of features
n_features <- length(setdiff(names(train_data), c("id", "address_number", "street", "overhead_utilities", "overhead_numeric")))

rf_overhead <- ranger(
  formula = overhead_numeric ~ . -id -address_number -street -overhead_utilities -overhead_numeric -longitude -latitude,
  data = train_data,
...
#Save random forest model
write_rds(rf_overhead, "rf_overhead.rds")
```

```{r}

#Load saved model
rf_overhead <- read_rds("rf_overhead.rds")
...
```{r}
#Variable importance plot
vip::vip(rf_overhead, num_features=10)
```
...
test_pred_rf <- factor(test_pred_rf$predictions, levels = c(0, 1))

# Convert the actual labels to a factor as well
test_data$overhead_numeric <- factor(test_data$overhead_numeric, levels = c(0, 1))

confusionMatrix(test_pred_rf, test_data$overhead_numeric, positive = "1")

```

```{r eval=FALSE}
#NOT WORKING
# Calculate predicted probability of delay for each test observation:
pred_test <- predict(rf_overhead, test_data, type = "prob")[, 2]

# Classify each tree as having overhead utilities (1) or not (0) and convert result to a factor
# the predicted labels must be a factor before passing to confusionMatrix function
pred_label <- ifelse(pred_test > 0.5, 1, 0)
pred_label <- factor(pred_label, levels=c(0, 1))

confusionMatrix(pred_label, test_data$overhead_numeric, positive = "1")

# the test dataset is called test_data; the response column is called overhead_numeric (0/1 variable)
pred_test <- as_tibble(pred_test) %>%
  rename(predicted = value)
pred_test <- pred_test  %>%
  bind_cols(test_data %>% dplyr::select(overhead_numeric))

pred_test %>%
  roc_curve(
    truth = overhead_numeric,
    predicted,
    event_level = "second"
  ) %>%
  autoplot()
