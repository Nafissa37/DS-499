...
```
*A good? RMSE since the average monetary value of the total air benefits provided by a tree is approximately $7.*
```{r eval=FALSE}
...
write_rds(rf_overhead2, "rf_overhead2.rds")
```
...
rf_overhead2 <- read_rds("rf_overhead2.rds")
...
```{r}
# Calculate predicted probability of overhead presence for each test observation:
pred_probs <- predict(rf_overhead2, test_data)$predictions[, 2]

# Convert to tibble and rename column
pred_test <- tibble(predicted = pred_probs) %>%
...
pred_test %>%
  roc_auc(truth = overhead_numeric, predicted, event_level = "second")

```

*Slightly higher AUC than with logistic regression; AUC higher than 0.5.*
...
train_data$land_use <- as.factor(train_data$land_use)
test_data$land_use <- as.factor(test_data$land_use)
...
write_rds(rf_land_use2, "rf_land_use2.rds")
```
...
rf_land_use2 <- read_rds("rf_land_use2.rds")
...
```{r}
pred_test <- predict(rf_land_use1, test_data, type="response")

# Align factor levels before evaluation (since land_use was re-leveled before fitting the rf model)
preds <- factor(pred_test$predictions, levels = levels(test_data$land_use))
actuals <- factor(test_data$land_use, levels = levels(test_data$land_use))
...
# Calculate predicted probability of overhead presence for each test observation:
pred_probs <- predict(rf_land_use2, test_data)$predictions[, 2]

# Convert to tibble and rename column
pred_test <- tibble(predicted = pred_probs) %>%
  bind_cols(test_data %>% select(land_use))
...
# Calculate area under the ROC curve
pred_test %>%
  roc_auc(truth = land_use, predicted, event_level = "second")

```


*(WIP) Slightly higher AUC than with logistic regression; AUC higher than 0.5.*
