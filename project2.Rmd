...
```{r}
#Focusing only on 2 categories for land_use (Residential and Commerical/Industrial)
...
```{r}
# Calculate predicted probability of overhead presence for each test observation:
pred_probs <- predict(rf_land_use2, test_data_land_use)$predictions[, 2]

# Convert to tibble and make sure land_use is a factor
pred_test <- tibble(predicted = pred_probs) %>%
  bind_cols(test_data_land_use %>% select(land_use) %>%
